...
class SiteTree extends DataObject implements PermissionProvider,i18nEntityProvider,CMSPreviewable {
	...
	*/
	 ...
	 * updateCMSFields called on them.
	 * 
	 * This method takes a callback, to which will be passed the FieldList 
	 * generated by the parent class (specified by $parentClass), allowing
	 * user code to interact with it.
	 * 
	 * This function will ensure that once the callback is completed that
	 * extensions will call updateCMSFields on the FieldList, before returning
	 * this back to user code to allow further manipulation.
	 * 
	 * Calls to beforeExtendingCMSFields cannot be nested, and thus may not
	 * be added to multiple classes in the same hierarchy.
	 * 
	 * E.g.
	 * 
	 * <pre>
	 ...
	 *		});
	 * 
	 *		// after extending updateCMSFields we may want to interact with fields
	 *		// added by extensions.
	 ...
	 * </pre>
	 * 
	 * @param string $parentClass Name of parent class from which, after disabling
	 * CMS field extensions, cms field should be retrieved. get_parent_class()
	 * should be passed in from the user code, or the next parent that implements
	 * getCMSFields if not implemented by the direct parent.
	 * @param callable $callback The callback to which the parent FieldList will be
	 * passed before extensions are processed.
	 * @return FieldList The list of parent fields
	 */
	protected function beforeExtendingCMSFields($parentClass, $callback) {
		if(!self::$runCMSFieldsExtensions) {
			user_error('Calls to SiteTree::beforeExtendingCMSFields cannot be nested', E_USER_ERROR);
		}
		SiteTree::disableCMSFieldsExtensions();
		$fields = call_user_func(array($this, "{$parentClass}::getCMSFields"));
		$callback($fields);
