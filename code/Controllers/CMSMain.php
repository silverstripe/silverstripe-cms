...
class CMSMain extends LeftAndMain implements CurrentRecordIdentifier, PermissionProvider, Flushable, MemberCacheFlusher
{
    ...
    public function getEditForm($id = null, $fields = null): Form
    {
        ...
        // Add extra fields
        $deletedFromStage = $record->hasExtension(Versioned::class) && $record->hasStages() && !$record->isOnDraft();
        $fields->push(new HiddenField("ID", false, $id));
        if ($record->hasMethod('Link') || $record->hasMethod('AbsoluteLink')) {
            $absoluteLink = $record->hasMethod('AbsoluteLink') ? $record->AbsoluteLink() : Director::absoluteURL($record->Link());
            // Necessary for different subsites
            $fields->push(new HiddenField("AbsoluteLink", false, $absoluteLink)); // @TODO generalise!!
            ...
            if (!$deletedFromStage) {
                $stageLink = Controller::join_links($absoluteLink, '?stage=Stage');
                if ($stageLink) {
