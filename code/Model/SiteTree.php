...
class SiteTree extends DataObject implements PermissionProvider, i18nEntityProvider, CMSPreviewable, Resettable, Flushable, MemberCacheFlusher
{
    ...
    */
    ...
    public static function get_by_link($link, $cache = true)
    {
        ...
        if (!$sitetree
            && SiteTree::config()->get('nested_urls')
        ...
        if (!$sitetree) {
            $parentID = SiteTree::config()->get('nested_urls') ? 0 : null;
        ...
        if (!SiteTree::config()->get('nested_urls') || !count($parts ?? [])) {
            return $sitetree;
        }
    ...
    {
        if ($this->ParentID && SiteTree::config()->get('nested_urls')) {
            $parent = $this->Parent();
    ...
    */
    ...
    private function getGenerator(): string
    {
        ...
        if (SiteTree::config()->get('show_meta_generator_version')) {
    ...
    public function validURLSegment()
    {
        ...
        if (SiteTree::config()->get('nested_urls') && $this->ParentID) {
    ...
    public function getCMSFields()
    {
        ...
        $baseLink = Controller::join_links(
            ...
            (SiteTree::config()->get('nested_urls') && $this->ParentID ? $this->Parent()->RelativeLink(true) : null)
        );
        ...
        $helpText = (SiteTree::config()->get('nested_urls') && $this->numChildren())
